...
#include <time.h>
#define millis(t) (clock()/(CLOCKS_PER_SEC / 1000))
#define radians(d) (d * (float) aPI / 180)
#define degrees(r) (r * ((float) 180) / (float) aPI)
...
void TinyGPS::f_get_position(float *latitude, float *longitude, unsigned long *fix_age)
{
  ...
  *latitude = (lat == GPS_INVALID_ANGLE) ? GPS_INVALID_F_ANGLE : (float) ((double)lat / 100000.0);
  *longitude = (lat == GPS_INVALID_ANGLE) ? GPS_INVALID_F_ANGLE : (float) ((double)lon / 100000.0);
}
...
{
  return _altitude == GPS_INVALID_ALTITUDE ? GPS_INVALID_F_ALTITUDE : ((float)_altitude) / 100.0f;
}

float TinyGPS::f_course()
{
  return _course == GPS_INVALID_ANGLE ? GPS_INVALID_F_ANGLE : ((float)_course) / 100.0f;
}

float TinyGPS::f_speed_knots() 
{
  return _speed == GPS_INVALID_SPEED ? GPS_INVALID_F_SPEED : ((float)_speed) / 100.0f;
}
